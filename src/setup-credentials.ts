...
function encryptPasswordDPAPI(password: string): string {
  try {
...
async function fetchApplications(baseUrl: string, token: string): Promise<OrgApplication[]> {
  try {
    console.log('📋 Fetching available applications...');
    const response = await axios.get<OrgApplication[]>(`${baseUrl}/api/applications`, {
      headers: {
...
function filterTicketingApps(apps: OrgApplication[]): OrgApplication[] {
  return apps.filter(app => app.Type === 'Ticketing' && app.Active);
}

function getAppsByIds(allApps: OrgApplication[], appIds: number[]): OrgApplication[] {
  return appIds.map(id => allApps.find(app => app.AppID === id)).filter((app): app is OrgApplication => app !== undefined);
}

async function selectApplications(apps: OrgApplication[], currentAppIds?: number[]): Promise<number[]> {
...
async function selectEnvironment(): Promise<string> {
  ...
  while (true) {
    ...
    if (choice === '1') return 'prod';
    if (choice === '2') return 'dev';
    if (choice === '3') return 'canary';
...
async function configureGlobal(mcpServerPath: string, credentialsPath: string, environment: string): Promise<void> {
...
async function configureProject(mcpServerPath: string, credentialsPath: string, environment: string): Promise<void> {
  ...
  let finalMcpConfig: typeof mcpConfig | null = mcpConfig;
...
async function main() {
  ...
  try {
    ...
    if (existsSync(credentialsPath)) {
      ...
      const existingCreds: ExistingCredentials = JSON.parse(readFileSync(credentialsPath, 'utf8'));
