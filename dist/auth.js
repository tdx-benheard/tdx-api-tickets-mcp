...
export class TDXAuth {
    ...
    async refreshToken() {
        ...
        try {
            const response = await axios.post(`${this.config.baseUrl}/api/auth`, {
            ...
            if (axios.isAxiosError(error)) {
                if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
                    throw new Error(`Authentication timeout after 10s. Check that the TeamDynamix server is accessible at ${this.config.baseUrl}`);
                }
