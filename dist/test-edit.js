import { readFileSync, existsSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
import { TDXClient } from './client.js';
// Load environment variables from .env file
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
// Check for credentials file
const credsFile = process.env.TDX_CREDENTIALS_FILE;
if (credsFile && existsSync(credsFile)) {
    const credsContent = readFileSync(credsFile, 'utf8');
    const creds = JSON.parse(credsContent);
    Object.assign(process.env, creds);
}
else {
    const envPath = join(__dirname, '..', '.env');
    if (existsSync(envPath)) {
        const envFile = readFileSync(envPath, 'utf8');
        for (const line of envFile.split('\n')) {
            const trimmed = line.trim();
            if (!trimmed || trimmed.startsWith('#'))
                continue;
            const equalIndex = trimmed.indexOf('=');
            if (equalIndex === -1)
                continue;
            const key = trimmed.slice(0, equalIndex).trim();
            let value = trimmed.slice(equalIndex + 1).trim();
            if ((value.startsWith('"') && value.endsWith('"')) ||
                (value.startsWith("'") && value.endsWith("'"))) {
                value = value.slice(1, -1);
            }
            if (!process.env[key]) {
                process.env[key] = value;
            }
        }
    }
}
const TDX_BASE_URL = process.env.TDX_BASE_URL || '';
const TDX_USERNAME = process.env.TDX_USERNAME || '';
const TDX_PASSWORD = process.env.TDX_PASSWORD || '';
const TDX_APP_ID = process.env.TDX_APP_ID || '';
const appIds = TDX_APP_ID.split(',').map(id => id.trim()).filter(id => id.length > 0);
console.log('🔧 Testing Edit Operations\n');
console.log('Base URL:', TDX_BASE_URL);
console.log('App IDs:', appIds.join(', '));
console.log('---\n');
const client = new TDXClient(TDX_BASE_URL, TDX_USERNAME, TDX_PASSWORD, appIds);
async function testPatchUpdate() {
    console.log('📝 Testing PATCH /api/627/tickets/554096');
    try {
        const result = await client.updateTicket(554096, {
            ResponsibleUid: 'a5a5fde1-f337-ec11-981e-0003ff5034fa',
            Tags: ['1-day', 'add', 'accounts', 'Analyzed by claude'],
            Comments: 'Testing PATCH update with ResponsibleUid and Tags'
        }, '627');
        console.log('✅ PATCH succeeded');
        console.log('   Responsible:', result.ResponsibleFullName);
        console.log('   Tags:', result.Tags?.join(', '));
        return true;
    }
    catch (error) {
        console.error('❌ PATCH failed:', error.response?.status, error.message);
        if (error.response?.data) {
            console.error('   Response:', JSON.stringify(error.response.data).slice(0, 500));
        }
        return false;
    }
}
async function testAddFeed() {
    console.log('\n💬 Testing POST /api/627/tickets/554096/feed');
    try {
        const result = await client.addTicketFeedEntry(554096, {
            Comments: 'Hello',
            IsPrivate: false
        }, '627');
        console.log('✅ Feed entry added');
        return true;
    }
    catch (error) {
        console.error('❌ Feed entry failed:', error.response?.status, error.message);
        if (error.response?.data) {
            console.error('   Response:', JSON.stringify(error.response.data).slice(0, 500));
        }
        return false;
    }
}
async function runTests() {
    const results = {
        patch: await testPatchUpdate(),
        feed: await testAddFeed(),
    };
    console.log('\n---');
    console.log('📈 Test Results:');
    console.log(`   PATCH Update:      ${results.patch ? '✅' : '❌'}`);
    console.log(`   Add Feed Entry:    ${results.feed ? '✅' : '❌'}`);
    const passed = Object.values(results).filter(Boolean).length;
    const total = Object.values(results).length;
    console.log(`\n${passed}/${total} tests passed`);
    process.exit(passed === total ? 0 : 1);
}
runTests().catch(error => {
    console.error('\n💥 Fatal error:', error);
    process.exit(1);
});
//# sourceMappingURL=test-edit.js.map